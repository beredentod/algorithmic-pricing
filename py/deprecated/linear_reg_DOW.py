...
df_agg = pd.read_csv('../data/MUC_oct22_aggregate.csv')
...
def generateLinRegCompany(company):
	if company != 'aggregate':
	...
	if company == 'jet':
		ax.errorbar(coefficients, x_labels, xerr=errors, label=company, linestyle='--', marker='|', markeredgecolor='k', ecolor='k', color=sc.hex_to_rgb(sc.colors_brands[company]))
		return
	
	if company == 'aggregate':
		ax.errorbar(coefficients, x_labels, xerr=errors, label=company, linestyle='-', marker='|', markeredgecolor='k', ecolor='k', color='green')
		return
...
generateLinRegCompany('aggregate')
generateLinRegCompany('aral')
generateLinRegCompany('shell')
generateLinRegCompany('agip')
generateLinRegCompany('esso')
generateLinRegCompany('jet')
generateLinRegCompany('omv')
...
ax.set_title('Regression Results for DOW - Comparison')
ax.legend(loc='right')
